                            ...
                            <v-row class="p-2 m-2 ">
                                ...
                                <v-col cols="12" md="6" class="p-2 m-1">
                                    <v-file-input
                                        ...
                                        label="Imagine Curs"
                                        accept="image/*"
                                        :rules="[isLenOfFileValid,rules.imageRequiredOnCreate]"
                                ...
                                <v-col cols="12" class="text-center" >
                                    <v-btn color="success" size="large" @click="finalUpdate()">Salvează
...
import { mdiShape, mdiAlphaDBoxOutline, mdiArrowDown, mdiArrowUp, mdiPen, mdiTrashCanOutline, mdiArrowRight, 
    mdiPlus, mdiAlphaX, mdiFileDocumentPlus, mdiCog, mdiFileDocumentMultipleOutline,mdiContentSave, 
    mdiNull} from '@mdi/js';
...
useHead({
    title: 'Modificare anunt'
})

const isAdding = "MODIFICARE ANUNT";
...
const announceId = route.params.idAnnounce
...
const rules = reactive({
    ...
    imageRequiredOnCreate: value => {
        // Dacă o imagine există deja (avem un URL), regula trece.
        if (announceData.value.presignedUrl && announceData.value.presignedUrl !== 'empty') {
            return true;
        }
        // Altfel, trebuie selectat un fișier nou (verificăm dacă 'value' nu e null/empty).
        return !!value || 'Imaginea este obligatorie.';
    }
});

watch(() => announceData.image, (newFileArray) => {
    const file = newFileArray ? newFileArray[0] : null;

    if (file) {

        if (announceData.presignedUrl && announceData.presignedUrl.startsWith('blob:')) {
            URL.revokeObjectURL(announceData.presignedUrl);
        }

        announceData.presignedUrl = URL.createObjectURL(file);
    }
});
...
const finalUpdate = (async () => {
    ...
    if(JSON.stringify(oldAnnounceData.value) === JSON.stringify(announceData.value)){
        console.log('aici222222')
        fireAlarm('warning' , 'Atentie' , 'Nu ati modificat nimic' , null, 2000 , 'center')
        return
    }

    const form = new FormData()
    const addURL = 'admin/announce/modify'
    const method = 'PUT'
    ...
    try{
        ...
        swal.close()
        fireAlarm('success' , 'Success' , 'Modificat cu success. Redirectionare' , '',2000,'top-end')
        setTimeout(() => {
...
const getAnnounceById = (async () => {
    fireAlarm('info' , 'Asteptati..' , '' , true)
    const getURL = `admin/announce/${announceId}`
    const method = 'GET'
    ...
    if(error.value === undefined){
        announceData.value = data.value
        console.log(data.value)
        announceData.value.presignedUrl = data.value.presignedUrl
        announceData.value.comingDate = data.value.comingDate.split('T')[0]
        announceData.value.leavingDate = data.value.leavingDate.split('T')[0]
        announceData.value.expirationDate = data.value.expirationDate.split('T')[0]
        announceData.value.image = null
        oldAnnounceData.value = JSON.parse(JSON.stringify(announceData.value))
        console.log(announceData.value)

    }else{
        if(error.value.statusCode === 404){
            fireAlarm('error' , 'Eroare' ,  'Anuntul nu mai exista. Redirectionare', null , 2000 , 'center')
            setTimeout(() => {
                navigateTo('/admin/announces')
            }, 3000);
        }else{
            fireAlarm('error' , 'Eroare' ,  'Eroare generala', null , 2000 , 'center')
        }
    }
})
